...
The Stakers Union uses the <SnapshotLink href={'/'}>Snapshot</SnapshotLink> platform to submit and vote on proposals. To submit a proposal, you must have an account on Snapshot.
...
Verified members can choose to delegate their voting power to a delegatee. The Stakers Union uses the [`"delegation"`](https://v1.snapshot.box/#/strategy/delegation) voting strategy, which allows members to delegate their voting power to another verified member without losing the ability to vote themselves if they choose to do so.

<div className={'mt-4'}>
  <Link href={'/governance/delegation'}>
    <Button>Delegation Instructions</Button>
  </Link>
</div>
